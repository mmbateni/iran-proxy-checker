...
=================================================
FIX: Robust network loading to prevent "0 prefixes" error.
FIX: IP sanitization for leading zeros.
"""
...
def sanitize_ip(ip_str):
    """Strips leading zeros (e.g., 103.250.166.04 -> 103.250.166.4)."""
    try:
...
def load_routable_networks():
    """Loads Iranian IP ranges from hardcoded fallback + local JSON."""
    # Robust fallback: Major Iranian IP blocks
    nets = [
        ipaddress.IPv4Network("5.160.0.0/12"), 
        ipaddress.IPv4Network("31.24.0.0/14"), 
        ipaddress.IPv4Network("37.254.0.0/15"), 
        ipaddress.IPv4Network("62.60.0.0/15"), 
        ipaddress.IPv4Network("77.36.0.0/14"),
        ipaddress.IPv4Network("78.38.0.0/15"),
        ipaddress.IPv4Network("151.232.0.0/13"),
        ipaddress.IPv4Network("188.121.0.0/16")
    ]
    
    if ASN_JSON_PATH.exists():
        log(f"[*] Loading additional networks from {ASN_JSON_PATH}...")
        try:
            with open(ASN_JSON_PATH) as f:
                data = json.load(f)
                cidrs = data.get("cidr_list", []) if isinstance(data, dict) else data
                for c in cidrs:
                    try: nets.append(ipaddress.IPv4Network(c, strict=False))
                    except: pass
        except Exception as e:
            log(f"  [!] JSON load error: {e}")
    
    unique_nets = list(set(nets))
    log(f"[*] Total active Iranian IP prefixes: {len(unique_nets)}")
    return unique_nets
...
def fetch_raw_url(url):
    try:
        ...
        return {f"{sanitize_ip(ip)}:{port}" for ip, port in found if not PRIVATE_RE.match(ip)}
    except:
        return set()

def collect_passive_candidates() -> set:
...
def main():
    ...
    passive_list = []
    for p_str in passive_candidates:
        try:
            ip_obj = ipaddress.IPv4Address(p_str.split(":")[0])
