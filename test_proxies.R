...
# [PATCH 3] Guard against empty output after trimming
valid_lines <- raw[nzchar(trimws(raw))]
if (length(valid_lines) == 0L) next

last_line  <- trimws(tail(valid_lines, 1L))
...
# [PATCH 4] Guard against empty output after trimming
valid_lines <- raw[nzchar(trimws(raw))]
...
# [PATCH 2] Ensure parsed JSON is a list before accessing keys
...
# [PATCH 1] Fixed broken string concatenation syntax
